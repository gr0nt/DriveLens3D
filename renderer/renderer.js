...
// 'gradient' mode renders a HORIZONTAL sweep — completely different from the
// vertical sky gradient above.  Vivid central band fades to dark edges.
const THEME_GRADIENT_H_STOPS = {
  neon:   [[0,'#0a0020'],[0.35,'#4400aa'],[0.5,'#6600ff'],[0.65,'#4400aa'],[1,'#0a0020']],
  ember:  [[0,'#0d0100'],[0.35,'#661500'],[0.5,'#cc2a00'],[0.65,'#661500'],[1,'#0d0100']],
  nature: [[0,'#010a02'],[0.35,'#016020'],[0.5,'#00aa44'],[0.65,'#016020'],[1,'#010a02']],
  ocean:  [[0,'#000820'],[0.35,'#004499'],[0.5,'#0077ee'],[0.65,'#004499'],[1,'#000820']],
  aurora: [[0,'#000810'],[0.35,'#006644'],[0.5,'#00cc88'],[0.65,'#006644'],[1,'#000810']],
};

function makeGradientTexture(themeName) {
  const stops = THEME_GRADIENT_H_STOPS[themeName];
  if (!stops) return null;
  const cvs = document.createElement('canvas');
  cvs.width = 512; cvs.height = 512;
  const ctx = cvs.getContext('2d');
  // Horizontal linear gradient — visually opposite to Sky's vertical gradient
...
// A persistent canvas updated every animation frame with falling katakana /
// digit characters, rendered as a Three.js scene.background texture.
const MATRIX_CHARS = 'ｦｧｨｩｪｫｬｭｮｯｰｱｲｳｴｵｶｷｸｹｺｻｼｽｾｿﾀﾁﾂﾃﾄﾅﾆﾇﾈﾉﾊﾋﾌﾍﾎﾏﾐﾑﾒﾓﾔﾕﾖﾗﾘﾙﾚﾛﾜﾝ0123456789ABCDEF';
let matrixCanvas = null, matrixCtx = null, matrixTex = null, matrixDrops = [];
const MATRIX_FONT_PX = 14, MATRIX_SIZE = 512;

function initMatrixCanvas() {
  matrixCanvas = document.createElement('canvas');
  matrixCanvas.width = matrixCanvas.height = MATRIX_SIZE;
  matrixCtx = matrixCanvas.getContext('2d');
  matrixCtx.fillStyle = '#000';
  matrixCtx.fillRect(0, 0, MATRIX_SIZE, MATRIX_SIZE);
  const cols = Math.floor(MATRIX_SIZE / MATRIX_FONT_PX);
  matrixDrops = Array.from({ length: cols }, () => Math.floor(Math.random() * (MATRIX_SIZE / MATRIX_FONT_PX)));
  matrixTex = new THREE.CanvasTexture(matrixCanvas);
}

function updateMatrixFrame() {
  if (!matrixCtx) return;
  // Fade existing characters with a semi-transparent black layer
  matrixCtx.fillStyle = 'rgba(0,0,0,0.06)';
  matrixCtx.fillRect(0, 0, MATRIX_SIZE, MATRIX_SIZE);
  const cols = matrixDrops.length;
  for (let i = 0; i < cols; i++) {
    // Bright head character
    const headY = matrixDrops[i] * MATRIX_FONT_PX;
    matrixCtx.font = `bold ${MATRIX_FONT_PX}px monospace`;
    matrixCtx.fillStyle = '#aaffaa';
    matrixCtx.fillText(MATRIX_CHARS[Math.floor(Math.random() * MATRIX_CHARS.length)], i * MATRIX_FONT_PX, headY);
    // Dim trailing character one step back
    if (matrixDrops[i] > 1) {
      matrixCtx.fillStyle = 'rgba(0,180,60,0.5)';
      matrixCtx.fillText(MATRIX_CHARS[Math.floor(Math.random() * MATRIX_CHARS.length)], i * MATRIX_FONT_PX, headY - MATRIX_FONT_PX);
    }
    // Reset column when it goes off-screen, randomly
    if (headY > MATRIX_SIZE && Math.random() > 0.97) matrixDrops[i] = 0;
    else matrixDrops[i]++;
  }
...
// 'sky'      — vertical gradient canvas texture (matches active theme palette)
// 'gradient' — horizontal gradient canvas texture (vivid centre, dark edges)
// 'matrix'   — animated Matrix-rain canvas texture (updated each frame)
...
let settings = {
  ...
  recursionDepth: 3,
  // labelFontSize: controls 3D label plane scale (56 = original size)
  labelFontSize: 56,
};
...
// Apply VM-safe capability overrides before the drive list loads so the
// first auto-render uses a mode that works without GPU acceleration.
(async () => {
  try {
    const caps = await window.api.getRuntimeCapabilities();
    if (caps && caps.vmSafe) {
      // Force a simple flat mode — treemap/city/galaxy rely heavily on WebGL depth
      currentMode = 'bar';
      document.querySelectorAll('.mode-btn').forEach(b =>
        b.classList.toggle('active', b.dataset.mode === 'bar')
      );
      // Hide GPU-heavy modes that will be unreliable without hardware acceleration
      document.querySelectorAll('[data-mode="city"], [data-mode="galaxy"]').forEach(el => {
...
function applyViewportBg(themeName) {
  ...
  // ── Solid: flat theme colour ─────────────────────────────────────────────
  if (viewportBgMode === 'solid') {
    scene.background = new THREE.Color(t.bg);
    if (renderer3) renderer3.setClearColor(t.bg, 1);
    return;
  }

  // ── Matrix rain: animated canvas updated each frame ──────────────────────
  if (viewportBgMode === 'matrix') {
    if (!matrixTex) initMatrixCanvas();
    scene.background = matrixTex;
    if (renderer3) renderer3.setClearColor(0x000000, 1);
    return;
  }

  // ── Sky: vertical gradient matching the active theme ─────────────────────
  if (viewportBgMode === 'sky') {
    ...
    // Aurora theme gets the animated CSS band overlay
    if (themeName === 'aurora') {
  ...
  // ── Gradient: horizontal sweep — vivid centre, dark edges (distinct from Sky) ─
  if (viewportBgMode === 'gradient') {
    const gradTex = makeGradientTexture(themeName);
    if (gradTex) {
      scene.background = gradTex;
      if (renderer3) renderer3.setClearColor(0x000000, 1);
    } else {
      // Cosmos / custom themes fall back to solid
      scene.background = new THREE.Color(t.bg);
      if (renderer3) renderer3.setClearColor(t.bg, 1);
    }
  }
...
(function initSettingsModal() {
  ...
  const sfv = document.getElementById('set-font-size-val');
  const sa  = document.getElementById('set-anim-speed');
  const sap = document.getElementById('set-anim-pct');
  const srd = document.getElementById('set-recursion-depth');
  ...
  function syncUI() {
    ...
    if (sfs) { sfs.value = settings.labelFontSize ?? 56; sfv.textContent = sfs.value + 'px'; }
  ...
  if (sl) sl.addEventListener('input', () => {
    ...
    saveSettings();
    scheduleLabelRebuild(); // debounced — avoids WebGL state corruption on rapid drag
  });
  if (sfs) sfs.addEventListener('input', () => {
    settings.labelFontSize = parseInt(sfs.value, 10);
    sfv.textContent = sfs.value + 'px';
    saveSettings();
    scheduleLabelRebuild(); // rebuild labels at new size, debounced
  });
...
// Debounced: rebuilding the scene on every pixel of slider drag caused rapid-fire
// geometry create/dispose cycles that corrupted WebGL bloom state permanently.
// We wait 120 ms after the last movement before rebuilding; also skip if a
// navigation transition is still mid-flight.
let _labelRebuildTimer = null;
function scheduleLabelRebuild() {
  clearTimeout(_labelRebuildTimer);
  _labelRebuildTimer = setTimeout(() => {
    if (!tAnim.active && navStack.length) showLevel(navStack[navStack.length - 1]);
  }, 120);
}
...
scanBtn.addEventListener('click', async () => {
  ...
  window.api.onScanProgress(({ pct, done, total }) => {
    ...
    // Mirror percentage in the status bar and viewport overlay for real-time feedback
    statusEl.textContent = `Scanning… ${pct}%`;
    showVpOverlay(`Scanning ${driveVal.replace(/\\+$/, '')}… ${pct}%`);
  });
...
function makeLabel(text, font) {
  ...
  // Always render the canvas at a fixed high resolution for sharpness.
  // The plane geometry is scaled by labelFontSize / 56 so the label appears
  // larger or smaller in the 3D world without sacrificing texture quality.
  const RENDER_FS  = 56;
  const planeScale = (settings.labelFontSize ?? 56) / 56;
  ctx.font = `bold ${RENDER_FS}px "${font}", sans-serif`;
  const tw = Math.max(ctx.measureText(text).width + 24, 80);
  canvas.width  = Math.ceil(tw);
  canvas.height = RENDER_FS + 16;
  ctx.font = `bold ${RENDER_FS}px "${font}", sans-serif`;
  // subtle shadow for depth
  ctx.shadowColor = 'rgba(0,0,0,0.8)'; ctx.shadowBlur = 6; ctx.shadowOffsetY = 2;
  ctx.fillStyle = 'rgba(255,255,255,0.92)';
  ctx.fillText(text, 12, RENDER_FS);
  const tex    = new THREE.CanvasTexture(canvas);
  const aspect = canvas.width / canvas.height;
  const pH     = 3.5 * planeScale;
  const mat    = new THREE.MeshBasicMaterial({ map: tex, transparent: true, depthWrite: false, side: THREE.DoubleSide });
  const geo    = new THREE.PlaneGeometry(aspect * pH, pH);
...
function renderSunburst(node) {
  ...
  // Ring geometry: [innerR, outerR, height] for the first five depth levels
  const RINGS = [
    [14, 36, 7],
    [37, 58, 5],
    [59, 76, 3],
  ...
  // Returns [innerR, outerR, height] for any depth level.
  // Levels beyond RINGS.length auto-generate outward rings so 'All' works.
  function getRingForDepth(d) {
    if (d <= RINGS.length) return RINGS[d - 1];
    const extra = d - RINGS.length;           // how many levels past the last defined ring
    const [, lastOR] = RINGS[RINGS.length - 1];
    const iR = lastOR + extra * 12;
    const oR = iR + 10;
    return [iR, oR, 1];
  }

  // Recursive sector builder — walks the tree up to maxDepth levels deep.
  // When maxDepth is Infinity ('All' mode) ring geometry is auto-generated
  // outward for every level present in the data.
  function buildRings(children, parentAngleStart, parentAngleSpan, depth) {
    if (depth > maxDepth) return;
    const [iR, oR, ht] = getRingForDepth(depth);
...
function animate() {
  ...
  // Update matrix rain canvas texture every frame when that mode is active
  if (viewportBgMode === 'matrix') updateMatrixFrame();
